...
const ROUTINES_DIR = path.join(PUBLIC_API_DIR, 'routines');
...
const sports = db.prepare(`
  SELECT id, name, icon, slug,
    warmup_duration as warmupDuration,
    stretch_duration as stretchDuration
  FROM sports ORDER BY id
`).all();

fs.writeFileSync(
  path.join(PUBLIC_API_DIR, 'sports.json'),
  JSON.stringify(sports),
  'utf-8'
);
console.log(`✅ sports.json — ${sports.length} 项运动`);

// ===== 2. 生成 age-groups.json =====
const ageGroups = [
...
fs.writeFileSync(
  path.join(PUBLIC_API_DIR, 'age-groups.json'),
  JSON.stringify(ageGroups),
  'utf-8'
);
console.log(`✅ age-groups.json — ${ageGroups.length} 个年龄段`);

// ===== 3. 生成每个 routine JSON =====
const coachMessages: Record<string, Record<string, string>> = {
  '6-8': {
...
function getCoachMessage(sportName: string, ageGroup: string): string {
  if (coachMessages[ageGroup]?.[sportName]) {
    return coachMessages[ageGroup][sportName];
  }
  if (coachMessages[ageGroup]?.['日常锻炼']) {
    return coachMessages[ageGroup]['日常锻炼'];
  }
  return `💪 我是小酷教练！今天我们要为「${sportName}」做热身准备，认真完成每一个动作哦！准备好了吗？开始吧！`;
}

function generateRoutine(sport: any, ageGroup: string, type: 'warmup' | 'stretch') {
  ...
  return {
    ...
    coachMessage: getCoachMessage(sport.name, ageGroup),
    steps: rows,
  };
}

// 遍历所有运动 × 年龄段 × 类型组合
type SportRow = { id: number; name: string; icon: string; slug: string; warmupDuration: string; stretchDuration: string };

let generatedCount = 0;
const manifest: Array<{ slug: string; age: string; type: string; sportName: string; stepCount: number }> = [];

for (const sport of sports as SportRow[]) {
  for (const age of ageGroups) {
    for (const type of ['warmup', 'stretch'] as const) {
      const routine = generateRoutine(sport, age.value, type);
      if (!routine) continue;

      const filename = `${sport.slug}_${age.value}_${type}.json`;
      fs.writeFileSync(
        path.join(ROUTINES_DIR, filename),
...
// 生成 routine manifest（前端可用它来检查哪些组合存在）
fs.writeFileSync(
  path.join(PUBLIC_API_DIR, 'routine-manifest.json'),
  JSON.stringify(manifest),
  'utf-8'
);

console.log(`✅ routines/ — ${generatedCount} 个方案文件`);
console.log(`✅ routine-manifest.json — ${manifest.length} 条索引`);
